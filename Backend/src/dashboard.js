...
dashBoardRouter.get('/get-number-of-rent-payments', async (req, res) => {
  try{
    ...
    FROM (
      SELECT type, description, amount, time, paidAmount
    ...
    WHERE final.type = 'Charge' AND final.paidAmount=0 AND final.description = 'Rent' AND final.time >= '${formattedStartDate}' AND final.time <= '${formattedEndDate}'`;
