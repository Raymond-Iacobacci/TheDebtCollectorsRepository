...
managerRouter.get("/get-outstanding-balances", async (req, res) => {
  try {
    const managerID = req.query["manager-id"];
    const tenantsList = await executeQuery(
...
managerRouter.get("/get-total-balances", async (req, res) => {
  try {
    const managerID = req.query["manager-id"];
    const tenantsList = await executeQuery(
      `SELECT tenantID, firstName, lastName from tenants where managerID = ${"0x" + managerID}`
    ...
    const topBalances = balances.sort((a, b) => b.balance - a.balance);
    const totalBalance = topBalances.reduce((sum, tenant) => sum + tenant.balance, 0);
